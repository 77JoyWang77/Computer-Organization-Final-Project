...
mkdir -p ${OUTPUTS_PATH}q2/${TASK}
(cd ${GEM5_PATH} && ${CMD} > ${OUTPUTS_PATH}q2/${TASK}/outputs 2> ${OUTPUTS_PATH}q2/${TASK}/errors)
echo "${CMD}" > ${OUTPUTS_PATH}q2/${TASK}/commands
cp ${GEM5_PATH}m5out/stats.txt ${OUTPUTS_PATH}q2/${TASK}/stats.txt
cp ${GEM5_PATH}m5out/config.ini ${OUTPUTS_PATH}q2/${TASK}/config.ini
